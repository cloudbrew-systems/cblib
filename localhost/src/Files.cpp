...
{
	std::string cmd;
	cmd += "rm -f " + this->FILES_TEMPDIR + "/" + userid + "/" + splitfilename;
	system(cmd.c_str());
	return CB_SUCCESS;
}

/* Encrypt the file shreds */
std::string Files::encryptshreds(std::string filename, std::string type, std::string userid, std::string originalfilenamehash, int totallinkedaccounts)
{
	// char cmd[MAX_CMD_LEN*2] = {'\0'}, passPhrase[MAX_LEN+1] = {'\0'}, 
	// fileEncryptedStateJSON[MAX_LEN*10] = {'\0'}, 
	// fileEncryptedStateJSONSingle[MAX_LEN*2] = {'\0'}, 
	// accountNameCopy[MAX_LEN] = {'\0'};

	// FILE *fp;
	// char *accountName, *shredFileName_chopped, 
	// *shredFileNameAccountName_chopped, *shredFileNameAccountIndex_chopped;
	...
	std::string cmd;
	/* Get a list of all the file shreds */
